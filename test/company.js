...
const BadgerDAOTemplate = artifacts.require('BadgerDAOTemplate')
...
contract('Company', ([_, owner, holder1, holder2, someone]) => {
  ...
  before('fetch company template and ENS', async () => {
    ens = await getENS()
    template = BadgerDAOTemplate.at(await getTemplateAddress())
  })

  const newInstance = (...params) => {
    const lastParam = params[params.length - 1]
    const txParams = (!Array.isArray(lastParam) && typeof lastParam === 'object') ? params.pop() : {}
    const newInstanceFn = BadgerDAOTemplate.abi.find(({ name, inputs }) => name === 'newInstance' && inputs.length === params.length)
